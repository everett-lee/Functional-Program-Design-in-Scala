...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    val validNeighbours = b.legalNeighbors
    val neighbourPairs = LazyList[(Block, List[Move])]()

    (validNeighbours foldLeft neighbourPairs) ((acc, pair) => {
      val (block, move) = pair
      (block, move :: history) #:: acc
    })
  }
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    neighbors filter { case (block, moves) =>
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty) LazyList.empty
      ...
      val more = for {
        (block, history) <- neighbours
        nextNeighbours <- newNeighborsOnly(neighborsWithHistory(block, history), explored + block)
      } yield nextNeighbours

      val newExplored = (neighbours map { case (block, moves) => block }).toSet
      neighbours #::: from(more sortBy { case (block, moves) => moves.length }, newExplored)
    }
  }
  ...
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = {
    pathsFromStart filter { case (block, moves) => block.b1 == goal && block.b2 == goal }
  }
  ...
  lazy val solution: List[Move] = {
    val result = pathsToGoal
    if (result.isEmpty) List() else result.head._2
  }
}
